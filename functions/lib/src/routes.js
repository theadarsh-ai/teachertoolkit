...
Remember, learning is a journey, and every question leads to new discoveries!`,
    ...
    app.get('/api/agents/knowledge-base/history/:userId', async (req, res) => {
        try {
            ...
            const history = await storage.getKnowledgeBaseHistory(parseInt(userId), parseInt(limit), parseInt(offset));
            res.json({
                success: true,
                data: history,
    ...
    app.get('/api/agents/knowledge-base/search/:userId', async (req, res) => {
        try {
            ...
            const results = await storage.searchKnowledgeBaseHistory(parseInt(userId), query, subject, grade ? parseInt(grade) : undefined);
            res.json({
                success: true,
                data: results,
    ...
    app.get('/api/sketchfab/models', async (req, res) => {
        try {
            ...
            const models = await sketchfabService.searchEducationalModels(query || 'education');
            res.json({
                success: true,
                count: models.length,
                data: models
