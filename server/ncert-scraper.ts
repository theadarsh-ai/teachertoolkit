...
export class NCERTScraper {
  ...
  private readonly NCERT_LESSONS_DATABASE = {
    // Grade 1 Mathematics
    1: {
      Mathematics: [
  ...
  async getLessonsBySubjectAndGrade(subject: string, grade: number): Promise<NCERTLesson[]> {
    try {
      console.log(`🔍 Fetching NCERT lessons for ${subject} Grade ${grade}`);
      
      const gradeData = this.NCERT_LESSONS_DATABASE[grade as keyof typeof this.NCERT_LESSONS_DATABASE];
      if (!gradeData) {
        console.log(`❌ No data found for Grade ${grade}`);
        return [];
      }

      const subjectLessons = gradeData[subject as keyof typeof gradeData];
      if (!subjectLessons) {
        console.log(`❌ No lessons found for ${subject} in Grade ${grade}`);
        return [];
      }

      const lessons: NCERTLesson[] = subjectLessons.map((lesson: any) => ({
        id: lesson.id,
        title: lesson.title,
  ...
  async getLessonContent(lessonId: string): Promise<NCERTLesson | null> {
    try {
      ...
      const gradeData = this.NCERT_LESSONS_DATABASE[gradeNum as keyof typeof this.NCERT_LESSONS_DATABASE];
      if (!gradeData) return null;

      const subjectLessons = gradeData[subject as keyof typeof gradeData];
      if (!subjectLessons) return null;

      const lesson = subjectLessons.find((l: any) => l.id === lessonId);
      if (!lesson) return null;

      // Generate detailed lesson content
      const detailedLesson: NCERTLesson = {
        id: lesson.id,
        title: lesson.title,
        chapter: lesson.chapter,
        subject: subject,
