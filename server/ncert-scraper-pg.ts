...
import { ncertStorage } from './ncert-storage';
...
export class NCERTScraper {
  ...
  async scrapeAllTextbooks(): Promise<{ textbooks: NCERTBookInfo[], scrapedCount: number }> {
    console.log('🚀 Starting comprehensive NCERT textbook scraping');
  ...
  async storeScrapedTextbooks(textbooks: NCERTBookInfo[]): Promise<number> {
    console.log(`💾 Storing ${textbooks.length} textbooks in PostgreSQL database...`);
    
    let storedCount = 0;
    
    for (const textbook of textbooks) {
      try {
        const stored = await ncertStorage.storeTextbook({
        ...
        console.error(`Failed to store: ${textbook.bookTitle}`, error);
      }
    }
    
    console.log(`✅ Successfully stored ${storedCount} textbooks in database`);
    return storedCount;
  }
}

// Export function for use in routes
export async function scrapeNCERTTextbooks() {
  const scraper = new NCERTScraper();
  const result = await scraper.scrapeAllTextbooks();
  const storedCount = await scraper.storeScrapedTextbooks(result.textbooks);
